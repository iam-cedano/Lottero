...
function authenticate(req: Request, res: Response, next: NextFunction) {
    if (!API_SECRET_KEY) return next(); // Skip if no secret key is configured
...
// Broadcast endpoint
app.post('/api/broadcast', authenticate, async (req: Request<{}, {}, BroadcastRequest>, res: Response) => {
...
app.listen(PORT, () => {
    ...
    if (!BOT_TOKEN) console.warn('⚠️  WARNING: TELEGRAM_BOT_TOKEN is not set.');
    if (!DEFAULT_CHANNEL_ID) console.warn('⚠️  WARNING: TELEGRAM_CHANNEL_ID is not set.');
    if (API_SECRET_KEY) console.log('🔒 API is protected with a secret key.');
});
